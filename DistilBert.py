...
class DistilBert:
    ...
    def bert_tokenize (self,X_train,X_val,X_test,max_len):
        print("Tokenizing")
        bert_tokenizer = DistilBertTokenizer.from_pretrained('distilbert-base-uncased')
        Xtrain_enc = bert_tokenizer(X_train.tolist(), max_length=max_len,
                                    truncation=True, padding='max_length', 
                                    add_special_tokens=True, return_tensors='np')
        Xval_enc = bert_tokenizer(X_val.tolist(), max_length=max_len,
                                  truncation=True, padding='max_length', 
                                  add_special_tokens=True, return_tensors='np')
        Xtest_enc = bert_tokenizer(X_test.tolist(), max_length=max_len,
                                   truncation=True, padding='max_length', 
    ...
    def bert_model(self,train_dataset,val_dataset,epochs,max_len):
        epochs=self.epochs
    ...
    def fit_distil_bert(self, sets,epochs,max_len):
        X_train, X_val, y_train, y_val = self.train_val_split(sets['X_train'],sets['y_train'])
        Xtrain_enc,Xval_enc,Xtest_enc =self.bert_tokenize(X_train,X_val,sets['X_test'],max_len)
        train_dataset, val_dataset, test_dataset =self.prepare_datasets(Xtrain_enc,Xval_enc,Xtest_enc,
                                                                        y_train,y_val,sets['y_test'])
        model = self.bert_model(train_dataset,val_dataset,epochs=epochs,max_len=max_len)
